function generateSignature(url, expires, secretKey,type) {
    var canonicalString = generateCanonicalString(url, expires, type);
    var signature =  b64_hmac_sha1(secretKey, canonicalString);
    return encodeURIComponent(signature); 
}

function generateCanonicalString(url, expires,type) {
    return type+"\n\n\n" + expires + "\n" + url;
}
...
function getSingedUrl(bucketName, accessKey, secretKey, params, type){
  var ttl= 8789798;
  bucketName="/"+bucketName;
  var secondsSinceEpoch = new Date().getTime() / 1000;
  var expiresAt = Math.floor(secondsSinceEpoch + parseInt(ttl, 10));
  var signature = generateSignature(bucketName, expiresAt, secretKey,type);
  return generateSignedUrl(bucketName, expiresAt, accessKey,
      signature)+queryStringCreation(params);
}
